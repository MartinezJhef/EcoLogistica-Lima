...
| Entidad | Descripción |
|---|---|
| **USUARIO** | Representa a las personas que acceden al sistema y permite administrar sus credenciales, roles y estados de acceso. |
| **CONDUCTOR** | Contiene el perfil operativo del conductor vinculado con un usuario del sistema. |
| **VEHICULO** | Registra las unidades de transporte disponibles, sus capacidades, características y factor de emisión. |
| **DEPOSITO** | Representa los puntos logísticos desde donde se inicia y finaliza la operación de una ruta. |
| **PEDIDO** | Representa una solicitud de entrega con información del cliente, ubicación, peso, volumen, prioridad y ventana horaria. |
| **RUTA** | Representa el recorrido planificado para atender pedidos, asignado a un conductor y vehículo en una fecha determinada. |
| **TRAMO** | Representa cada segmento de recorrido que forma parte de una ruta, manteniendo el orden de desplazamiento y sus métricas. |
| **ZONA_RESTRINGIDA** | Representa áreas geográficas donde existen restricciones de circulación, obras, condiciones especiales o criterios ambientales. |
| **INCIDENCIA** | Registra eventos ocurridos durante la operación, como tráfico, ausencia del cliente, averías u otras situaciones. |
| **REPORTE_EMISIONES** | Almacena los reportes periódicos relacionados con las emisiones de CO2 y el ahorro estimado generado por la optimización. |
| **DETALLE_REPORTE_RUTA** | Tabla intermedia que relaciona los reportes de emisiones con las rutas utilizadas para generar sus indicadores. |
...
- Un **VEHICULO** puede estar asignado a múltiples **RUTAS** (1:N).
- Un **DEPOSITO** puede ser utilizado por múltiples **RUTAS** (1:N).
...
- Una **RUTA** puede estar asociada a múltiples **REPORTES_EMISIONES** y viceversa mediante **DETALLE_REPORTE_RUTA** (N:M).
- Un **TRAMO** puede intersectar espacialmente una o múltiples **ZONAS_RESTRINGIDAS**.
